...
<label style={cameraBtn}><Camera size={18}/> {compra.foto ? "ARCHIVO LISTO ✅" : "ADJUNTAR FACTURA"} <input type="file" accept="image/*,.pdf" hidden onChange={e=>setCompra({...compra, foto:e.target.files[0]})} /></label>            <button onClick={registrarGasto} style={{...btn, background: COLOR_PIKHN}} disabled={loading}>{loading ? "PROCESANDO..." : "REGISTRAR"}</button>
          </div>
        )}
